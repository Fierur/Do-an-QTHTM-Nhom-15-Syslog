...
for CLIENT_ID in 1 2 3; do
    ...
    if [[ $DUPLICATES -gt 0 ]]; then
        echo -e "    ${RED}  Duplicate : $DUPLICATES seq number bị ghi 2 lần${NC}"
...
if [[ $SWITCHES -ge 10 ]]; then
    ...
    echo -e "    ${YELLOW}  Interleaving TRUNG BÌNH — Có xen kẽ nhưng chưa đều ($SWITCHES lần)${NC}"
else
...
if [[ "$ALL_OK" == "true" && $TOTAL_RECEIVED -eq $EXPECTED_TOTAL ]]; then
    ...
    echo ""
    echo -e "    ${YELLOW}${BOLD}  Có sai lệch — xem chi tiết bên trên${NC}"
